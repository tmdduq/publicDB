...
public class SoundSearcher {
    ...
    /**
     * 검색을 한다. 초성 검색 완벽 지원함. * @param value : 검색 대상 ex> 초성검색합니다
     * * @param search : 검색어 ex> ㅅ검ㅅ합ㄴ * @return 매칭 되는거 찾으면 true 못찾으면 false.
     */
    public boolean matchString(String value, String search) {

        search = getInitialSound(search);
        int t = 0;
    ...
    protected String getInitialSound(String text) {
        ...
        if (text.length() > 0) {
            for (int i = 0; i < text.length(); i++) {
                ...
                if (chName >= 0xAC00 && chName <= 0xD7A3) {  // 0xAC00(가) ~ 0xD7A3(힣)
                    ...
                    result +=initialChs[initialCh];
                } else {
                    result = result+chName;
                }
            }
        }
        return result;
